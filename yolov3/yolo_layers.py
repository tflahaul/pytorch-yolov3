...
class YoloDetectionLayer(torch.nn.Module):
	...
	def forward(self, inputs: torch.Tensor) -> torch.Tensor:
		...
		self.out = inputs.reshape(samples, self.__nb_anchors, self.__bbox_attrs, y, x).permute(0, 1, 3, 4, 2).contiguous()
		self.scaled_anchors = self.__anchors.div(stride).to(inputs.device)
